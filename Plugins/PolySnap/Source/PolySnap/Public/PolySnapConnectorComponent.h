...
#include "PolySnapConnectorComponent.generated.h"

class UMeshComponent;

/**
 * Makes an actor a PolySnap part: parses the edge sockets off its mesh once, caches them, and
 * records what they are connected to.
 *
 * The component is what connects the actor into an assembly, which is where its name comes from,
 * and an actor carrying one is a part. PolySnap ships no actor class to inherit from, so a project
 * keeps its own actor hierarchy and adds this to whatever it already has -- see DESIGN section 2.11. It
 ...
 * component never turns simulation on; it only forces kinematic when the part is anchored.
 */
UCLASS(ClassGroup = "PolySnap", meta = (BlueprintSpawnableComponent), DisplayName = "PolySnap Connector")
class POLYSNAP_API UPolySnapConnectorComponent : public UActorComponent
{
	GENERATED_BODY()

public:
	UPolySnapConnectorComponent();
	...
	/** The mesh whose sockets this part owns. Null means "the owner's first mesh component". */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "PolySnap")
	TObjectPtr<UMeshComponent> SocketMesh;

	/**
	 * Keeps this part kinematic, so it stays put and gives the player something fixed to build
	...
	/**
	 * Tick when this part's mesh was authored to a different socket axis convention than the
	 * project default. Leave it off and the part follows UPolySnapSettings::DefaultSocketAxes.
	 */
	UPROPERTY(EditAnywhere, Category = "PolySnap", meta = (InlineEditConditionToggle))
	bool bOverrideSocketAxes = false;

	/** Which socket-local axis carries which role on this part's mesh. CONVENTIONS.md section 2. */
	UPROPERTY(EditAnywhere, Category = "PolySnap", meta = (EditCondition = "bOverrideSocketAxes"))
	FPolySnapSocketAxes SocketAxes;

	/** The convention in force: this part's override if it has one, otherwise the project default. */
	...
	/** The parsed edge sockets on this part, in the order the mesh lists them. */
	...
	/** True while this part is a fixed reference: it never simulates and cannot be picked up. */
	[[nodiscard]] bool IsAnchored() const { return bStartAnchored; }

	/**
	 * Applies UPolySnapSettings' damping and sleep thresholds to this part's mesh body.
	 *
	 * Every part needs these or it drifts away in zero gravity, and a part is whatever actor
	 * carries this component, so this is the only place that can apply them. Called at BeginPlay,
	 * whenever the settings change, and whenever the part re-enters the simulation.
	 */
	void ApplyPhysicsSettings();

	/**
	 * Hands the part to the simulation, or takes it out so it can be carried.
	 *
	 * The one place that knows what handing a part back involves: an anchored part refuses, the
	...
	/** Every socket on this part, resolved into world space, appended to OutSockets. */
	 ...
	 * part's own convention is applied; a caller that goes to the mesh component directly gets
	 * raw axes and silently wrong snapping.
	 */
	[[nodiscard]] FTransform GetSocketWorldTransform(FName SocketName) const;

	/** The part's own transform -- its owning actor's. What a solved placement is expressed in. */
	[[nodiscard]] FTransform GetPartTransform() const;

	/** True when this exact socket pair is already connected, in either direction. */
	[[nodiscard]] bool IsConnectedTo(const UPolySnapConnectorComponent* Other, int32 OtherSocketId,
	...
	/** Records one side of a connection. The caller records the other side on the other part. */
	...
	/** Finds a cached descriptor by socket ID. Null when this part has no such socket. */
...
private:
	...
	/** Subscription to UPolySnapSettings, so a retune reaches a part that is already simulating. */
